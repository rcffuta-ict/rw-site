...
export function CartSidebar({ onClose }: { onClose: () => void }) {
    ...
    return (
        <>
            ...
            <aside className="fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col bg-white shadow-2xl animate-slide-in-right" aria-label="Cart">
                <div className="flex items-center justify-between border-b border-[var(--rw-border)] px-6 py-4 sticky top-0 bg-white">
                    <h2 className="font-display font-bold text-lg text-rw-ink">
                        Shopping Cart <span className="font-normal text-sm text-rw-muted">({itemCount})</span>
                    </h2>
                ...
                {items.length === 0 ? (
                    <div className="flex flex-1 flex-col items-center justify-center gap-5 p-8 text-center">
                        ...
                        <Button variant="outlined" size="md" onClick={onClose}>Browse Merch</Button>
                    </div>
                ) : (
                    <>
                                    ...
                                    <div className="flex flex-1 flex-col gap-1.5 min-w-0">
                                        ...
                                        <div className="flex items-center gap-2 mt-1">
                                            <div className="flex items-center border border-[var(--rw-border)] rounded-lg overflow-hidden">
                                                <button onClick={() => updateQuantity(item.variantId, item.quantity - 1)} className="h-7 w-7 flex items-center justify-center text-rw-ink hover:bg-rw-bg-alt text-xs font-bold">−</button>
                                                <span className="w-7 text-center text-xs font-bold text-rw-ink border-x border-[var(--rw-border)]">{item.quantity}</span>
                                                <button onClick={() => updateQuantity(item.variantId, item.quantity + 1)} className="h-7 w-7 flex items-center justify-center text-rw-ink hover:bg-rw-bg-alt text-xs font-bold">+</button>
                                            </div>
                                            <button onClick={() => removeItem(item.variantId)} className="ml-auto text-xs text-rw-muted hover:text-rw-crimson transition-colors font-medium">Remove</button>
                                        </div>
                                    </div>
                                </li>
                            ))}
                        ...
                        <div className="border-t border-[var(--rw-border)] p-5 flex flex-col gap-4 bg-white sticky bottom-0">
                            <div className="flex justify-between items-center">
                                <span className="text-sm font-medium text-rw-text-2">Subtotal</span>
                                <span className="font-display font-bold text-xl text-rw-ink">₦{total.toLocaleString()}</span>
                            </div>
