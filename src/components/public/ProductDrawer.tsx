...
export function ProductDrawer({ product, onClose }: { product: Product; onClose: () => void }) {
    ...
    const allSizes = [...new Set(product.variants.filter(v => v.size).map(v => v.size!))].sort(
    ...
    const variant: ProductVariant | undefined = product.variants.find(
        v => (selectedColor === null || v.color === selectedColor)
            ...
            <aside className="fixed inset-y-0 right-0 z-50 w-full max-w-lg bg-white shadow-2xl animate-slide-in-right flex flex-col overflow-hidden">
                ...
                <div className="flex items-center justify-between border-b border-[var(--rw-border)] px-6 py-4 shrink-0">
                    <div>
                        ...
                        <p className="text-xs text-rw-muted mt-0.5">Select your options below</p>
                    </div>
                    <button
                        onClick={onClose}
                        aria-label="Close"
                ...
                <div className="flex-1 overflow-y-auto">
                        ...
                        {selectedColor && (
                            <div className="absolute bottom-4 left-4 flex items-center gap-2 bg-white/90 backdrop-blur-sm rounded-full px-3 py-1.5 shadow-sm">
                                <span
                                    ...
                                    style={{ background: COLOR_HEX[selectedColor] ?? "#888" }}
                                />
                                <span className="text-xs font-semibold text-rw-ink">{selectedColor}</span>
                            </div>
                        )}
                        {/* Dimensions badge */}
                        <div className="absolute top-3 right-3 bg-black/30 backdrop-blur-sm rounded-lg px-2 py-1">
                            <span className="text-[10px] font-mono text-white/80">512×384</span>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-7 p-6">
                        ...
                        <div>
                            ...
                            <p className="mt-2.5 text-sm text-rw-text-2 leading-relaxed">{product.description}</p>
                        </div>
                        ...
                        {colors.length > 0 && (
                            <div>
                                <div className="flex items-center justify-between mb-3">
                                    <p className="text-sm font-semibold text-rw-ink">Colour</p>
                                    {selectedColor && (
                                        <span className="text-sm text-rw-text-2">{selectedColor}</span>
                                    )}
                                </div>
                                        ...
                                        >
                                            {selectedColor === c && (
                                                <span className="absolute inset-0 flex items-center justify-center">
                                                    <svg className="h-4 w-4 text-white drop-shadow" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
                        ...
                        {allSizes.length > 0 && allSizes[0] !== "One Size" && (
                            <div>
                                <div className="flex items-center justify-between mb-3">
                                    <p className="text-sm font-semibold text-rw-ink">Size</p>
                                    {selectedSize && (
                                        <span className="text-sm text-rw-text-2">{selectedSize}</span>
                                    )}
                                </div>
                        ...
                        {!available && variant && (
                            <div className="flex items-center gap-2 rounded-xl bg-amber-50 border border-amber-200 px-4 py-3">
                                ...
                                <p className="text-sm text-amber-700 font-medium">This variant is currently out of stock.</p>
                            </div>
                        )}
                    </div>
                </div>
