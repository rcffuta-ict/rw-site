...
export function AdminNotification({
    ...
    return (
        <div
            className={`h-10 w-full ${current.bg} ${current.text} px-4 flex items-center justify-center gap-3 text-[10px] font-bold uppercase tracking-[0.2em] z-50 border-b border-white/10 shadow-lg relative`}
        >
            <svg
                className={`h-3 w-3 ${current.iconColor} animate-pulse`}
                fill="currentColor"
                viewBox="0 0 24 24"
            >
            ...
            <span className="opacity-90">
                <span className={current.iconColor}>{current.label}:</span>{" "}
