        ...
        <div className="min-h-[80vh] flex flex-col items-center justify-center section-container py-12 animate-fade-in-up">
            <div className="max-w-md w-full text-center mb-8">
                <div className="inline-flex h-20 w-20 items-center justify-center rounded-[2rem] bg-rw-crimson/10 border border-rw-crimson/20 shadow-sm mb-8">
                    <svg className="h-10 w-10 text-rw-crimson" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
                    </svg>
                </div>
                <h1 className="section-heading text-4xl mb-4">Lookup Order</h1>
                <p className="text-rw-text-2 text-lg">Enter your 6-character reference code to securely pay for your order.</p>
            </div>
            ...
            <div className="w-full max-w-md bg-white rounded-[2rem] p-8 sm:p-10 shadow-2xl shadow-rw-ink/5 border border-[var(--rw-border)]">
                <div className="relative">
                    <input
                        type="text"
                        value={refInput}
                        onChange={(e) => setRefInput(e.target.value.toUpperCase().replace(/[^A-F0-9]/g, '').slice(0, 6))}
                        placeholder="000000"
                        maxLength={6}
                        className="w-full h-20 bg-rw-bg-alt border-2 border-[var(--rw-border-mid)] rounded-2xl text-center font-mono text-4xl tracking-[0.3em] text-rw-ink uppercase focus:border-rw-crimson focus:ring-4 focus:ring-rw-crimson/10 transition-all placeholder:text-rw-muted/30"
                    />
                </div>
                {notFound && (
