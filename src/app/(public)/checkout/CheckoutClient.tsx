                ...
                return (
                    <div key={label} className="flex items-center">
                        <div className="flex items-center gap-2.5">
                            <span className={`flex h-9 w-9 items-center justify-center rounded-full text-sm font-bold transition-all ${
                                done  ? "bg-rw-crimson text-white"
                                : active ? "border-2 border-rw-crimson text-rw-crimson"
                                : "border-2 border-gray-200 text-gray-400"
                            }`}>
                                {done ? "✓" : n}
                            </span>
                            <span className={`text-sm font-medium hidden sm:block ${active ? "text-rw-ink" : done ? "text-rw-ink" : "text-rw-muted"}`}>{label}</span>
                        </div>
                        {i < steps.length - 1 && <div className={`h-px w-10 sm:w-16 mx-3 ${done ? "bg-rw-crimson" : "bg-gray-200"}`} />}
                    </div>
                );
            })}
        </div>
...
function OrderSummaryPanel({ items, total }: { items: ReturnType<typeof useCart>["items"]; total: number }) {
    return (
        <div className="rw-card p-6 sticky top-24">
                ...
                {items.map(i => (
                    <li key={i.variantId} className="flex gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-rw-muted truncate">{i.variantLabel}</p>
                            <div className="flex justify-between mt-1">
                                <span className="text-xs text-rw-muted">Qty: {i.quantity}</span>
                                <span className="text-sm font-semibold text-rw-ink">₦{(i.unitPrice * i.quantity).toLocaleString()}</span>
                            </div>
                        </div>
                    </li>
                ))}
            ...
            <div className="border-t border-[var(--rw-border)] pt-4 flex justify-between items-center">
                ...
                <span className="font-display font-bold text-2xl text-rw-crimson">₦{total.toLocaleString()}</span>
            </div>
        </div>
    );
}
...
export function CheckoutClient() {
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            existing.push({
                orderRef: ref, customerName: form.name, customerEmail: form.email,
                customerPhone: form.phone, customerNote: form.note || null,
                status: "pending", totalAmount: total, amountPaid: 0,
                items: items.map(i => ({ id: crypto.randomUUID(), variantId: i.variantId, productName: i.productName, variantLabel: i.variantLabel, quantity: i.quantity, unitPrice: i.unitPrice })),
            ...
            const refs = JSON.parse(localStorage.getItem("rw_order_refs") ?? "[]") as string[];
            refs.push(ref);
    ...
    function field(id: string, label: string, type = "text", placeholder = "") {
        ...
        return (
            <div>
                <label htmlFor={id} className="block text-sm font-medium text-rw-ink mb-2">{label}</label>
        ...
        return (
            <div className="section-container py-20 text-center">
                <div className="max-w-md mx-auto">
                    ...
                    <p className="text-rw-muted mb-6">Add some merch before checking out.</p>
                    <Link href="/shop"><Button variant="primary" size="lg">Browse Merch</Button></Link>
                </div>
            </div>
        );
    }
                    ...
                    <div>
                        {step === 1 && (
                            <div className="rw-card p-6 sm:p-8 flex flex-col gap-5">
                                <h2 className="font-display font-bold text-lg text-rw-ink">Your details</h2>
                                {field("name",  "Full name",     "text",  "e.g. Adewale Ogundimu")}
                                {field("email", "Email address", "email", "you@example.com")}
                                {field("phone", "Phone number",  "tel",   "080...")}
                                <div>
                                    <label htmlFor="note" className="block text-sm font-medium text-rw-ink mb-2">
                        ...
                        {step === 2 && (
                            <div className="flex flex-col gap-5">
                                <div className="rw-card p-6 sm:p-8">
                                    <h2 className="font-display font-bold text-lg text-rw-ink mb-5">Order Review</h2>
                                        ...
                                        <tbody>
                                            {items.map(i => (
                                                <tr key={i.variantId} className="border-b border-[var(--rw-border)] last:border-0">
                                                    ...
                                                    <td className="py-4 text-right text-rw-text-2">{i.quantity}</td>
                                                    <td className="py-4 text-right font-semibold text-rw-ink">₦{(i.unitPrice * i.quantity).toLocaleString()}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                    ...
                    <div className="max-w-lg mx-auto flex flex-col gap-6 animate-fade-in-up">
                        <div className="rw-card p-10 text-center flex flex-col items-center gap-5">
                            ...
                            <div className="w-full rounded-2xl border-2 border-rw-crimson/20 bg-rw-bg-alt px-8 py-6 text-center">
                                <p className="font-mono text-5xl font-bold text-rw-ink tracking-[0.12em]">{orderRef}</p>
                                <p className="mt-2 text-xs text-rw-muted">Keep it safe — share it with whoever is paying.</p>
                            </div>
                            <div className="flex flex-wrap gap-3 justify-center">
                                ...
                                <Link href={`/fulfil?ref=${orderRef}`} id="go-to-fulfil">
                                    <Button variant="primary" size="md">Go to Payment →</Button>
                                </Link>
                            </div>
                        </div>
                        <p className="text-center text-xs text-rw-muted">
                            You can also pay later at <Link href="/fulfil" className="text-rw-crimson hover:underline">/fulfil</Link>
                        </p>
                    </div>
                )
            )}
