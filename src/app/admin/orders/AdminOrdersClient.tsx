...
export function AdminOrdersClient() {
  ...
  async function setStatus(orderId: string, status: OrderStatus) {
    ...
    setOrders((prev) => (prev ? prev.map((o) => (o.id === nextOrder.id ? nextOrder : o)) : prev));
  }
    ...
    <section className={styles.card}>
      ...
      {error ? (
        <div style={{ marginTop: 12, color: "#b42318", fontWeight: 800 }}>
          {error} (If you’re not logged in, go to `/admin/login`.)
        </div>
      ) : null}
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
                <div style={{ fontWeight: 900 }}>
                  {o.customerName}{" "}
                  <span style={{ color: "var(--rw-muted)", fontWeight: 800 }}>
                    ({o.phone})
                  </span>
                </div>
                <div style={{ fontWeight: 900, color: "var(--rw-primary)" }}>
                  ₦{o.totalNgn.toLocaleString()}
                </div>
              </div>

              <div style={{ marginTop: 6, color: "var(--rw-muted)", fontSize: 13 }}>
                {o.id} · {statusLabel(o.status)} · {new Date(o.createdAt).toLocaleString()}
              </div>
              ...
              <div style={{ marginTop: 10, color: "var(--rw-muted)", fontSize: 13 }}>
                Receipt:{" "}
                {o.receipt ? (
                  <span style={{ fontWeight: 800 }}>
                    {o.receipt.filename} (uploaded {new Date(o.receipt.uploadedAt).toLocaleString()}
                    )
                  </span>
                ) : (
              ...
              <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap" }}>
                <button onClick={() => setStatus(o.id, "confirmed")} style={miniBtn}>
                  Confirm
                </button>
                <button onClick={() => setStatus(o.id, "rejected")} style={miniBtn}>
                  Reject
                </button>
                <button onClick={() => setStatus(o.id, "fulfilled")} style={miniBtn}>
                  Fulfilled
                </button>
              </div>
            </div>
