        ...
        <div className="flex flex-col gap-8">
            <div className="flex items-end justify-between flex-wrap gap-4">
                <div>
                    ...
                    <p className="mt-1 text-sm text-rw-muted">
                        Generated fulfilment documents for confirmed orders
                    </p>
                </div>
                <Link
            ...
            <div className="rounded-2xl border border-blue-200 bg-blue-50 px-5 py-4 flex items-start gap-3">
                ...
                <p className="text-sm text-blue-700">
                    <strong>Verdicts</strong> are official documents sent to the
                    production team or used for order withdrawal. Only{" "}
                    <strong>Confirmed</strong> orders can be included. Demo build — PDF
            ...
            {MOCK_VERDICTS.length === 0 ? (
                <div className="rw-card flex flex-col items-center gap-4 py-16 text-center">
                    ...
                    <Link
                        href="/admin/verdicts/new"
                        className="text-sm font-semibold text-rw-crimson hover:underline"
                    >
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {MOCK_VERDICTS.map((v) => (
                        <div
                            key={v.id}
                            className="rw-card p-5 flex flex-col gap-4 hover:shadow-md transition-shadow"
                        >
                            {/* Header */}
                            <div className="flex items-start justify-between gap-2">
                                <span
                                    className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-semibold ${TYPE_COLORS[v.type] ?? "bg-gray-100 text-gray-600 border-gray-200"}`}
                                >
                                    {v.type}
                                </span>
                                <span className="text-[10px] font-mono text-rw-muted">
                                    {v.id.toUpperCase()}
                                </span>
                            </div>
                            ...
                            {/* Order refs */}
                            <div>
                                <p className="text-xs text-rw-muted mb-1.5 uppercase tracking-wider font-semibold">
                                    Orders
                                </p>
