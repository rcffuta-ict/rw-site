...
import { SidebarItem, VariableChip, PreviewPane, HeaderFooterEditor } from "./components";
...
export default function EmailTemplatesClient() {
    ...
    const handleLayoutUpdate = useCallback(
        (field: keyof SiteEmailLayout, value: string) => {
    ...
    return (
        <div className="h-full flex flex-col bg-white overflow-hidden">
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-(--rw-border) bg-white shrink-0">
                <div>
                    <h1 className="text-xl font-bold text-rw-ink">Email Templates</h1>
                    <p className="text-sm text-rw-muted mt-0.5">
                        Manage automated customer email templates
                    </p>
                </div>
                <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                        showSettings
                            ...
                            : "bg-rw-bg-alt text-rw-muted hover:text-rw-ink border border-(--rw-border)"
                    }`}
                >
            ...
            {showSettings ? (
                <div className="flex-1 overflow-y-auto p-6 bg-white">
                    <div className="max-w-2xl">
                        <h2 className="text-lg font-bold text-rw-ink mb-4">
                            Email Layout Settings
                        </h2>
                        <p className="text-sm text-rw-muted mb-6">
                            Configure the header and footer that appear on all email
                            templates.
                        </p>
                        <HeaderFooterEditor
                            layout={siteLayout}
                    ...
                    <div className="w-64 shrink-0 flex flex-col gap-4 overflow-y-auto">
                        ...
                        <div>
                            ...
                            <div className="flex flex-col gap-1">
                                {TEMPLATES.filter((t) => t.category === "order").map(
                                    (t) => (
                                        <SidebarItem
                                            key={t.key}
                                            template={t}
                        ...
                        <div>
                            ...
                            <div className="flex flex-col gap-1">
                                {TEMPLATES.filter((t) => t.category === "payment").map(
                                    (t) => (
                                        <SidebarItem
                                            key={t.key}
                                            template={t}
                    ...
                    <div className="flex-1 flex flex-col gap-4 min-w-0">
                        ...
                        <div className="flex items-center justify-between gap-4 pb-4 border-b border-(--rw-border)">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-base font-bold text-rw-ink">
                                        {activeTemplate?.label}
                                    </h2>
                                    <code className="text-xs text-rw-muted font-mono bg-rw-bg-alt px-2 py-1 rounded">
                                        {activeKey}
                                    </code>
                                </div>
                            </div>
                        ...
                        <div className="flex-1 flex gap-4 min-h-0">
                            ...
                            {(view === "editor" || view === "split") && (
                                <div
                                    className={`flex flex-col gap-3 ${view === "split" ? "flex-1" : "w-full"}`}
                                >
                                    {/* Subject */}
                                    <Input
                                        label="Subject Line"
                                    ...
                                    <div className="flex-1 flex flex-col min-h-0">
                                        <Textarea
                                            ...
                                            description="Supports HTML tags. Use the variable chips above to insert dynamic values."
                            ...
                            {(view === "preview" || view === "split") && (
                                ...
                                >
                                    <p className="text-xs font-bold text-rw-muted uppercase tracking-wider">
                                        Preview (Sample Data)
                                    </p>
