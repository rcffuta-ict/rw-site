    ...
    if (hasFilters) {
        return (
            <div className="rw-card p-20 flex flex-col items-center justify-center text-center gap-4 bg-rw-bg-alt/30 border-dashed">
                ...
                <div>
                    <p className="font-display font-bold text-xl text-rw-ink">
                        No products found
                    </p>
        ...
        <div className="rw-card p-20 flex flex-col items-center justify-center text-center gap-6 bg-rw-bg-alt/30 border-dashed">
            ...
            <div>
                ...
                <p className="text-sm text-rw-muted mt-2 max-w-xs mx-auto leading-relaxed">
                    Add your first product to start building the Redemption Week &apos;26
                    catalog.
                </p>
            </div>
            {isAdmin && (
        ...
        <div className="rw-card group flex flex-col bg-white border-none shadow-md ring-1 ring-[var(--rw-border)] hover:ring-rw-crimson/30 hover:shadow-xl transition-all duration-300 overflow-hidden">
            ...
            <div className="relative aspect-[3/4] overflow-hidden bg-rw-bg-alt">
                ...
                <div className="absolute top-3 right-3">
                    <span
                        className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[9px] font-bold uppercase tracking-wider backdrop-blur-md border ${
                            product.isAvailable
                    ...
                    >
                        <span
                            className={`h-1.5 w-1.5 rounded-full ${product.isAvailable ? "bg-green-500 shadow-[0_0_6px_rgba(34,197,94,0.6)]" : "bg-gray-400"}`}
                        />
                ...
                {product.tags.length > 0 && (
                    <div className="absolute bottom-3 left-3 flex gap-1">
                        {product.tags.slice(0, 2).map((tag) => (
                            <span
                                key={tag}
                                className="px-2 py-1 rounded-lg text-[9px] font-bold uppercase tracking-widest bg-rw-crimson/90 text-white backdrop-blur-sm shadow-sm"
                            >
                                {tag}
                            </span>
                        ))}
            ...
            <div className="p-4 flex flex-col flex-1 gap-3">
                ...
                <div className="flex items-center justify-between py-2 border-y border-[var(--rw-border)]">
                    <div className="flex flex-col gap-0.5">
                        ...
                        <span className="text-xs font-bold text-rw-ink">
                            {availableCount}/{product.variants.length}
                        </span>
                    </div>
                    <div className="flex flex-col items-end gap-0.5">
                        ...
                        <div className="flex gap-1 mt-0.5">
                            ...
                            {uniqueColors.length > 4 && (
                                <span className="text-[9px] font-bold text-rw-muted">
                                    +{uniqueColors.length - 4}
                                </span>
                            )}
                        </div>
                ...
                <div className="flex items-center gap-2 mt-auto">
                    ...
                    >
                        <span
                            className={`inline-block h-4 w-4 rounded-full bg-white shadow-sm transition-transform duration-300 ${product.isAvailable ? "translate-x-6" : "translate-x-1"}`}
                        />
                    </button>
                </div>
            </div>
    ...
    function handleToggle(productId: string, currentlyAvailable: boolean) {
        ...
        startTransition(async () => {
            ...
            if (!res.success) {
                ...
                setProducts((prev) =>
                    prev.map((p) =>
                        p.id === productId ? { ...p, isAvailable: currentlyAvailable } : p
                    )
                );
                toast.error("Failed to update visibility", { description: res.error });
            } else {
                toast.success(
                    next ? "Product is now live" : "Product hidden from storefront"
                );
            }
        });
    }
            ...
            icon: (
                ...
                >
                    <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M6 6.878V6a2.25 2.25 0 012.25-2.25h7.5A2.25 2.25 0 0118 6v.878m-12 0c.235.083.487.122.75.122h10.5c.263 0 .515-.039.75-.122m-12 0a3.75 3.75 0 00-3.75 3.75v1.5a3.75 3.75 0 003.75 3.75m12-9a3.75 3.75 0 013.75 3.75v1.5a3.75 3.75 0 01-3.75 3.75m-12 0c.235.083.487.122.75.122h10.5c.263 0 .515-.039.75-.122m-12 0V19.5a2.25 2.25 0 002.25 2.25h7.5a2.25 2.25 0 002.25-2.25v-4.122"
                    />
                </svg>
            ),
        },
