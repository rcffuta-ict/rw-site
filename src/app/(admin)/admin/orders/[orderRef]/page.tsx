...
interface Props { params: Promise<{ orderRef: string }>; }
    ...
    return (
        <div className="flex flex-col gap-8">
            ...
            <nav className="flex items-center gap-2 text-sm text-rw-muted">
                <Link href="/admin/orders" className="hover:text-rw-crimson transition-colors">Orders</Link>
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        <h1 className="font-mono font-bold text-3xl lg:text-4xl text-rw-ink">{order.orderRef}</h1>
                        <OrderStatusBadge status={order.status} />
                    </div>
                    <p className="text-sm text-rw-muted">
                        Created {new Date(order.createdAt).toLocaleString("en-NG", { dateStyle: "medium", timeStyle: "short" })}
                    </p>
                </div>
                <div className="flex flex-wrap gap-2">
                    <button className="btn-secondary !h-10 !px-5 text-sm">Confirm Order</button>
                    <button className="rounded-xl border border-red-200 px-5 py-2 text-sm font-semibold text-red-600 hover:bg-red-50 transition-colors">Cancel</button>
                </div>
            </div>
            ...
            <div className="grid lg:grid-cols-[1fr_320px] gap-8 items-start">
                <div className="flex flex-col gap-6">
                    ...
                    <div className="rw-card p-6">
                        <p className="text-[10px] font-bold text-rw-muted uppercase tracking-[0.15em] mb-3">Customer</p>
                        ...
                        {order.customerNote && (
                            <p className="mt-4 text-sm italic text-rw-muted bg-rw-bg-alt rounded-xl p-3 border border-[var(--rw-border)]">&ldquo;{order.customerNote}&rdquo;</p>
                        )}
                    </div>
                    ...
                    <div className="rw-card overflow-hidden">
                        <div className="px-6 py-4 border-b border-[var(--rw-border)]">
                            <p className="text-[10px] font-bold text-rw-muted uppercase tracking-[0.15em]">Line Items</p>
                        </div>
                        <div className="divide-y divide-[var(--rw-border)]">
                            {order.items.map((i) => (
                                <div key={i.id} className="flex items-center gap-4 px-6 py-4">
                                    ...
                                    <span className="text-sm text-rw-text-2 shrink-0">× {i.quantity}</span>
                                    <span className="font-semibold text-rw-ink text-sm shrink-0">{fmt(i.unitPrice * i.quantity)}</span>
                                </div>
                            ))}
                            <div className="flex justify-between px-6 py-4 bg-rw-bg-alt">
                                <span className="font-semibold text-rw-ink">Total</span>
                                <span className="font-bold text-rw-crimson text-lg">{fmt(order.totalAmount)}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-4">
                            <p className="font-display font-bold text-rw-ink">
                                Payment History {order.payments.length > 0 && `(${order.payments.length})`}
                            </p>
                        </div>
                        {order.payments.length === 0 ? (
                            <div className="rw-card p-10 text-center">
                                <p className="text-rw-muted">No payments submitted yet.</p>
                            </div>
                        ) : (
                            <div className="flex flex-col gap-4">
                                {order.payments.map((p) => (
                                    <PaymentReviewCard
                                        ...
                                        onFlag={(id, note) => console.log("Flag", id, note)}
                                        onReject={(id, note) => console.log("Reject", id, note)}
                                    />
                                ))}
                            </div>
                        )}
                ...
                <div className="flex flex-col gap-6">
                    <div className="rw-card p-6 flex flex-col gap-5">
                        <p className="text-[10px] font-bold text-rw-muted uppercase tracking-[0.15em]">Payment Summary</p>
                        <div>
                        ...
                        <dl className="flex flex-col gap-3 text-sm">
                            <div className="flex justify-between">
                                ...
                                <dd className="font-semibold text-rw-ink">{fmt(order.totalAmount)}</dd>
                            </div>
                            <div className="flex justify-between">
                                <dt className="text-rw-muted">Paid</dt>
                                <dd className="font-semibold text-green-700">{fmt(order.amountPaid)}</dd>
                            </div>
                            <div className="flex justify-between border-t border-[var(--rw-border)] pt-3">
                                <dt className="text-rw-muted">Remaining</dt>
                                <dd className="font-bold text-rw-crimson">{fmt(order.totalAmount - order.amountPaid)}</dd>
                            </div>
                        </dl>
                        <Link href={`/fulfil?ref=${order.orderRef}`} target="_blank" className="text-xs text-center text-rw-crimson hover:underline font-medium">
                            Open payment page ↗
                        </Link>
                    </div>
                    ...
                    <div className="rw-card p-6 flex flex-col gap-3">
                        <p className="text-[10px] font-bold text-rw-muted uppercase tracking-[0.15em]">Timeline</p>
                        <div className="text-sm text-rw-text-2 space-y-2">
                            <div className="flex items-center gap-3">
                                <span className="h-2 w-2 rounded-full bg-green-500 shrink-0" />
                                <span>Created {new Date(order.createdAt).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}</span>
                            </div>
                            <div className="flex items-center gap-3">
                                <span className="h-2 w-2 rounded-full bg-blue-500 shrink-0" />
                                <span>Updated {new Date(order.updatedAt).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}</span>
                            </div>
                        </div>
                    </div>
                </div>
