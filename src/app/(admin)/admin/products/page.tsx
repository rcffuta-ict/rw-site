            ...
            <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
                {DEMO_PRODUCTS.map((p) => {
                    ...
                    return (
                        <div
                            key={p.id}
                            className="rw-card overflow-hidden flex flex-col hover:shadow-md transition-shadow"
                        >
                            ...
                            >
                                ...
                                <div className="absolute top-3 right-3">
                                    <span
                                        className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold ${CATEGORY_COLORS[p.category] ?? "bg-gray-100 text-gray-600 border-gray-200"}`}
                                    >
                                ...
                                <div className="absolute bottom-0 left-0 right-0 px-4 py-3 bg-gradient-to-t from-black/60 via-black/20 to-transparent">
                                    <span
                                        className={`inline-flex items-center gap-1.5 text-xs font-semibold ${p.isAvailable ? "text-green-300" : "text-gray-400"}`}
                                    >
                                        <span
                                            className={`h-1.5 w-1.5 rounded-full ${p.isAvailable ? "bg-green-400 animate-pulse-soft" : "bg-gray-400"}`}
                                        />
                            ...
                            <div className="p-5 flex flex-col gap-4 flex-1">
                                <div className="flex items-start justify-between gap-2">
                                    <h2 className="font-display font-bold text-rw-ink text-lg leading-snug">
                                        {p.name}
                                    </h2>
                                    <span className="font-bold text-rw-crimson text-lg shrink-0">
                                        ₦{p.basePrice.toLocaleString()}
                                    </span>
                                </div>

                                <p className="text-sm text-rw-muted line-clamp-2 leading-relaxed">
                                    {p.description}
                                </p>
                                ...
                                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-rw-text-2">
                                    ...
                                    {uniqueSizes.length > 1 && (
                                        <span>Sizes: {uniqueSizes.join(" · ")}</span>
                                    )}
                                </div>
