        ...
        <div className="flex flex-col gap-8">
            ...
            <div className="grid lg:grid-cols-[300px_1fr] gap-6">
                <div className="rounded-2xl overflow-hidden bg-rw-bg-alt">
                    <img src={`https://placehold.co/300x240?text=${encodeURIComponent(product.name)}`} alt={product.name} className="w-full h-48 lg:h-full object-cover" />
                </div>
                <div className="flex flex-col gap-4">
                    <div>
                        <h1 className="section-heading text-2xl lg:text-3xl">{product.name}</h1>
                        <p className="mt-2 text-rw-text-2 leading-relaxed">{product.description}</p>
                    </div>
                    <p className="font-display font-bold text-3xl text-rw-crimson">₦{product.basePrice.toLocaleString()} <span className="text-sm font-normal text-rw-muted">base price</span></p>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        {[
                            ...
                            <div key={stat.label} className="rw-card p-3 text-center">
                                <p className="font-display font-bold text-xl text-rw-ink">{stat.value}</p>
                                <p className="text-[10px] text-rw-muted font-medium uppercase tracking-wider">{stat.label}</p>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="flex flex-wrap gap-3">
                        {uniqueColors.map(color => (
                            <div key={color} className="flex items-center gap-2 rounded-xl border border-[var(--rw-border)] px-3 py-2 bg-white">
                                <span className="h-5 w-5 rounded-full border border-black/10" style={{ backgroundColor: COLOR_HEX[color] ?? "#999" }} />
            ...
            <div>
                <div className="flex items-center justify-between mb-4">
                    <p className="font-display font-bold text-rw-ink">Variants</p>
                    <button onClick={() => alert("Add variant — stub in demo build")}
                ...
                <div className="rw-card overflow-hidden">
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {variants.map(v => (
                                <tr key={v.id} className={`border-b border-[var(--rw-border)] last:border-0 transition-colors ${v.isAvailable ? "hover:bg-rw-bg-alt/50" : "bg-gray-50/50 opacity-50"}`}>
                                    ...
                                    <td className="px-5 py-4 text-right font-semibold">
                                        {v.priceOverride ? <span className="text-rw-crimson">₦{v.priceOverride.toLocaleString()}</span> : <span className="text-rw-muted text-xs">base</span>}
                                    </td>
                ...
                <div className="mt-5 flex items-center gap-3">
                    <button onClick={handleSave} className="btn-primary !h-10 !px-5 text-sm">Save Changes</button>
                    {saved && (
