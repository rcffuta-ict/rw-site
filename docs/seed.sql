...
BEGIN
    ...
    SELECT id INTO cat_tshirt FROM categories WHERE slug = 'tshirt';
    SELECT id INTO cat_hoodie FROM categories WHERE slug = 'hoodie';
    SELECT id INTO cat_accessory FROM categories WHERE slug = 'accessory';

    -- 2. Insert Products
    INSERT INTO products (id, category_id, name, description, base_price, tags, is_available)
    VALUES 
        (prod_tshirt, cat_tshirt, 'Anniversary T-Shirt', 'Premium comfort fit, breathable cotton tailored for worship and warfare.', 4500, '{rw26,tshirt}', TRUE),
        (prod_hoodie, cat_hoodie, 'RW''26 Premium Hoodie', 'Warm, clean, and durable fleece. Built for the cold nights.', 12000, '{rw26,hoodie,winter}', TRUE),
        (prod_acc, cat_accessory, 'Redemption Sticker Pack', 'A small, fun add-on for your laptop, journal, or water bottle.', 800, '{rw26,accessory}', TRUE)
    ON CONFLICT (id) DO NOTHING;

    -- 3. Insert Variants
    INSERT INTO product_variants (id, product_id, size, color, color_hex, design, price_override, is_available)
    VALUES
    ...
    -- 4. Insert Images (Using Placehold.co placeholders styled like the demo)
    INSERT INTO product_images (variant_id, cloudinary_public_id, cloudinary_url, alt_text, is_primary)
    VALUES
